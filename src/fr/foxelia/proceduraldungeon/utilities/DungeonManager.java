...
public class DungeonManager {
	...
	public static boolean generateDungeon(DungeonManager dungeon, Location spawnLoc) {
		...
		// The current facing direction of the dungeon corridor.
		// Starts facing NORTH (the default entrance direction faces south, exit faces north)
		...
		for(int i = 0; i < dungeon.getConfig().getInt("roomcount"); i++) {
			...
			// Calculate rotation needed to align this room's entrance with the current direction.
			// The room's entrance direction tells us which face the entrance is on (in the room's local space).
			// We need to rotate the room so that its entrance faces opposite to currentDirection
			// (i.e., the entrance "opens towards" where we're coming from).
			Direction requiredEntranceFacing = currentDirection.opposite();
			int rotationDegrees = roomchoosen.getEntranceDirection().rotationTo(requiredEntranceFacing);
			...
			// Calculate the exit offset after rotation
			double exitX = roomchoosen.getExit().getX();
			double exitY = roomchoosen.getExit().getY();
			double exitZ = roomchoosen.getExit().getZ();

			// Rotate the exit offset by the same rotation applied to the schematic
			double[] rotatedExit = rotatePoint(exitX, exitZ, rotationDegrees);

			// Move the spawn location to the exit of this room
			spawnLoc.add(rotatedExit[0], exitY, rotatedExit[1]);

			// Update the current direction based on the room's exit direction, rotated
			currentDirection = roomchoosen.getExitDirection().rotate(rotationDegrees);
		}
		return true;
	}

	/**
	 * Rotate a 2D point (x, z) clockwise by the given degrees around the origin.
	 * Clockwise from above: NORTH(-Z) → EAST(+X) → SOUTH(+Z) → WEST(-X)
	 * @return double[]{newX, newZ}
	 */
	private static double[] rotatePoint(double x, double z, int degrees) {
		...
		// Round to avoid floating point drift on 90-degree increments
